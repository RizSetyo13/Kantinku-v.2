...
import { Store, Star, Check, Trash2 } from 'lucide-react';
          ...
          <table>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7}><div className="empty-state"><div className="empty-icon" style={{ opacity: 0.2 }}><Store size={48} /></div><h3>Tidak Ada Kedai</h3></div></td></tr>
              ) : filtered.map(k => {
                  ...
                  <tr key={k.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        <img src={k.image_url?.startsWith('/uploads/') ? `http://localhost:5000${k.image_url}` : (k.image_url || `https://picsum.photos/seed/${k.id}k/64/64`)} alt="" style={{ width: 40, height: 40, borderRadius: 8, objectFit: 'cover' }} onError={e => e.target.src = '/images/kedai_bakso.png'} />
                        <div>
                    ...
                    <td style={{ color: 'var(--accent)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <Star size={14} fill="#ffb400" style={{ color: '#ffb400' }} /> {parseFloat(k.rating || 0).toFixed(1)}
                    </td>
                    <td>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.35rem' }}>
                          <span className={`badge ${k.is_active ? 'badge-info' : 'badge-muted'}`}>{k.is_active ? 'Aktif' : 'Off'}</span>
