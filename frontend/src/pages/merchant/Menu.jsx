...
export default function MerchantMenu() {
  ...
  const save = async (e) => {
    ...
    try {
      ...
      formData.append('kedai_id', form.kedai_id);
      ...
      if (editing) { 
        await api.put(`/merchant/menu/${editing.id}`, formData, config); 
        toast.success('Menu diperbarui'); 
      } else { 
        await api.post('/merchant/menu', formData, config); 
  ...
  const ICONS = {
    makanan: <Utensils size={14} style={{marginRight:'4px'}} />,
    minuman: <Coffee size={14} style={{marginRight:'4px'}} />,
    snack: <Pizza size={14} style={{marginRight:'4px'}} />,
    edit: <Edit size={14} />,
      ...
      <div className="with-sidebar main-content">
        ...
        {filtered.length === 0 ? (
          <div className="empty-state">
            ...
            <p>Tambahkan menu untuk kedai kamu</p>
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map(m => (
                  <tr key={m.id}>
                    ...
                    <td><span className="badge badge-info" style={{display:'flex', alignItems:'center'}}>{ICONS[m.category]} {m.category}</span></td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <button className="btn btn-sm btn-secondary" onClick={() => open(m)} title="Edit">{ICONS.edit}</button>
                        <button className="btn btn-sm btn-danger" onClick={() => del(m.id)} title="Hapus">{ICONS.delete}</button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            <form onSubmit={save}>
              ...
              <div className="form-group">
                ...
                <small style={{ color: 'var(--text-muted)', fontSize: '0.7rem', marginTop: '0.25rem', display: 'block' }}>
                  Pilih file untuk mengupload foto baru. Biarkan kosong jika tidak ingin mengubah.
                </small>
              </div>
              <div className="form-group">
                <label className="form-label">URL Gambar (Luar)</label>
                <input className="form-control" placeholder="https://..." value={form.image_url} onChange={e => setForm({...form, image_url: e.target.value})} />
              </div>
