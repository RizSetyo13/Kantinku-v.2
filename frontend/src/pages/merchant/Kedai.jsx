...
import { Store, Clock, Star, Edit, X } from 'lucide-react';
    ...
    <div className="app-layout">
      ...
      <div className="with-sidebar main-content">
        <div className="page-header-row">
          ...
          <button className="btn btn-primary" onClick={() => openModal()}>+ Daftarkan Kedai</button>
        </div>

        <div style={{ marginTop: '1.5rem' }}>
          {kedai.length === 0 ? (
            <div className="empty-state">
              <div className="empty-icon" style={{ opacity: 0.2 }}><Store size={48} /></div>
              <h3>Belum ada kedai</h3>
              <p>Daftarkan kedai kamu untuk mulai berjualan</p>
              <button className="btn btn-primary" style={{ marginTop: '1rem' }} onClick={() => openModal()}>+ Daftarkan Sekarang</button>
            </div>
          ) : (
              ...
              {kedai.map(k => {
                ...
                return (
                  <div key={k.id} className="card">
                    <img src={k.image_url?.startsWith('/uploads/') ? `http://localhost:5000${k.image_url}` : (k.image_url || `https://picsum.photos/seed/${k.id}kd/400/200`)} alt={k.name} style={{ width: '100%', height: 160, objectFit: 'cover', borderRadius: 'var(--radius-md)', marginBottom: '1rem' }} onError={e => e.target.src = '/images/kedai_bakso.png'} />
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', alignItems: 'flex-end' }}>
                        ...
                        <span className={`badge ${isOpen ? 'badge-success' : 'badge-danger'}`} style={{ fontWeight: 800 }}>{isOpen ? '● BUKA' : '● TUTUP'}</span>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><Clock size={14} /> {k.open_time?.substring(0,5)} – {k.close_time?.substring(0,5)}</span>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><Star size={14} fill="#ffb400" style={{ color: '#ffb400' }} /> {parseFloat(k.rating || 0).toFixed(1)} ({k.review_count})</span>
                    </div>
                    <button className="btn btn-secondary btn-sm" onClick={() => openModal(k)} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', justifyContent: 'center' }}>
                      <Edit size={14} /> Edit Kedai
                    </button>
                  </div>
                );
              })}
            ...
            <form onSubmit={save}>
              ...
              {editing && (
                <div className="form-group" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <input type="checkbox" id="isActive" checked={form.is_active} onChange={e => setForm({...form, is_active: e.target.checked})} style={{ width: 18, height: 18, cursor: 'pointer' }} />
                  <label htmlFor="isActive" className="form-label" style={{ margin: 0, cursor: 'pointer' }}>Kedai Aktif</label>
                </div>
              )}
