...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    // --- KantinPay Integration ---
    const [wallets] = await conn.query('SELECT saldo FROM wallets WHERE user_id = ? FOR UPDATE', [req.user.id]);
    if (wallets.length === 0) {
      await conn.rollback();
      return res.status(400).json({ message: 'Anda belum memiliki wallet KantinPay' });
    }
    const currentBalance = parseFloat(wallets[0].saldo);
    if (currentBalance < total) {
      await conn.rollback();
      return res.status(402).json({ message: 'Saldo KantinPay tidak mencukupi' });
    }
    ...
    await conn.query('UPDATE wallets SET saldo = ? WHERE user_id = ?', [newBalance, req.user.id]);
    
    // Record payment transaction
    await conn.query(
      `INSERT INTO wallet_transactions (user_id, order_id, tipe, jumlah, saldo_setelah, keterangan)
       VALUES (?, ?, 'pembayaran', ?, ?, ?)`,
      [req.user.id, orderResult.insertId, total, newBalance, `Pembayaran pesanan #${order_code}`]
    );

    await conn.query(
      'INSERT INTO notifications (user_id, order_id, message) VALUES (?, ?, ?)',
      [req.user.id, orderResult.insertId,
      `✅ Pembayaran berhasil! Pesanan #${order_code} dikirim — menunggu konfirmasi kedai`]);
...
const createWebsiteReview = async (req, res) => {
  const { nama, no_telp, rating, keterangan } = req.body;
  ...
  try {
    await pool.query(
      'INSERT INTO ulasan_website (nama, no_telp, rating, keterangan) VALUES (?, ?, ?, ?)',
      [nama.trim(), no_telp.trim(), rating || 5, keterangan.trim()]);
    res.status(201).json({ message: 'Ulasan berhasil dikirim. Terima kasih!' });
  } catch (err) {
    res.status(500).json({ message: 'Server error', error: err.message });
  }
...
const getWebsiteReviews = async (req, res) => {
  try {
    const [rows] = await pool.query(`
      SELECT
        uw.id, uw.nama, uw.no_telp, uw.rating, uw.keterangan, uw.created_at,
...
// ─────────────────────────────────────────────────────────────────
// PROTECTED — WALLET (KantinPay)
// ─────────────────────────────────────────────────────────────────

const getWalletBalance = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT saldo FROM wallets WHERE user_id = ?', [req.user.id]);
    if (rows.length === 0) return res.status(404).json({ message: 'Wallet tidak ditemukan' });
    res.json({ balance: parseFloat(rows[0].saldo) });
  } catch (err) {
    res.status(500).json({ message: 'Server error', error: err.message });
  }
};
...
const topupWallet = async (req, res) => {
  ...
  if (!amount || amount <= 0) return res.status(400).json({ message: 'Nominal tidak valid' });
  ...
  try {
    ...
    await conn.query(
      `INSERT INTO wallet_transactions (user_id, tipe, jumlah, saldo_setelah, keterangan)
       VALUES (?, 'topup', ?, ?, 'Top-up KantinPay (Demo)')`,
      [req.user.id, amount, newBalance]
    );

    await conn.commit();
    res.json({ balance: newBalance, message: 'Top-up berhasil' });
  } catch (err) {
    await conn.rollback();
    res.status(500).json({ message: 'Server error', error: err.message });
  } finally {
...
module.exports = {
  ...
  getWalletBalance, getWalletTransactions, topupWallet
};
